...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener, FragmentCallback{
    ...
    private static String GEOCODE_URL="https://dapi.kakao.com/v2/local/search/keyword.json?query=";
    private static String GEOCODE_USER_INFO="ㄱㅐ인restkey";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportFragmentManager().beginTransaction().add(R.id.main, fragment2).commit();
        getHashKey();
    ...
    public void onFragmentSelected(int position, Bundle bundle) {
        ...
        if (position == 0) {
            ...
            toolbar.setTitle("어디쓰?!레기통!");
        } else if (position == 1) {
            curFragment = fragment2;
            toolbar.setTitle("분리배출 가이드");
        } else if (position == 2) {
        ...
        switch (requestCode){ //실행 후 전달받을 코드!
            case PERMISSIONS_REQUEST:
                if (grantResults.length>0 && grantResults[0]==PackageManager.PERMISSION_GRANTED){//권한이 granted
                    ...
                    Toast.makeText(this, "권한이 취소되었습니다.", Toast.LENGTH_LONG).show();}//권한 거절된 경우
                break;//switch문
        }
    }
