...
import static com.example.trash_2.MainActivity.PERMISSIONS_REQUEST;
...
public class MapFragment extends Fragment implements View.OnClickListener{
    ...
    private static String GEOCODE_USER_INFO="KakaoAK eb6ba968a461fc9c14049615d423377d";
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
       ...
       //mapView.setCurrentLocationTrackingMode(MapView.CurrentLocationTrackingMode.TrackingModeOnWithoutHeading); //현재 위치 추적
        ...
        switch (view.getId()){
            case R.id.location_button:
                if(ContextCompat.checkSelfPermission(
                        getActivity(),Manifest.permission.ACCESS_COARSE_LOCATION )==PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(getActivity(),Manifest.permission.BIND_ACCESSIBILITY_SERVICE)==PackageManager.PERMISSION_GRANTED){
                    test = false;
...
//                break;
                    ...
                    test = false;
                    addToCoordinate(editText.getText().toString(), test);
                    Toast.makeText(getActivity(),"위치권한을 거부하여 사용할 수 없는 기능입니다.[설정]에서 위치권한을 허용해주세요.",Toast.LENGTH_LONG).show();
                    break;
                }
//                test = false;
//                addToCoordinate(editText.getText().toString(),test);
        ...
        switch (requestCode){ //실행 후 전달받을 코드!
            case PERMISSIONS_REQUEST:
                if (grantResults.length>0 && grantResults[0]==PackageManager.PERMISSION_GRANTED){//권한이 granted
                    Toast.makeText(getActivity(), "권한이 설정되었습니다.", Toast.LENGTH_LONG).show();
                }//권한 허가된 경우
                else{
                    Toast.makeText(getActivity(), "권한이 취소되었습니다.", Toast.LENGTH_LONG).show();}//권한 거절된 경우
                break;//switch문
        }
    }
